...
<!-- v1.4, written by Jim Mangiafico, updated 22 May 2016 -->
...
<xsl:template name="hierarchy">
	...
	<xsl:element name="{$name}">
		...
		<xsl:if test="$name = 'level'">
			<xsl:attribute name="class"><xsl:value-of select="$hcontainer-name" /></xsl:attribute>
		</xsl:if>
	...
	<xsl:call-template name="hierarchy">
		<xsl:with-param name="name">
			<xsl:choose>
				...
				<xsl:when test="self::Pblock">level</xsl:when>
				<xsl:when test="self::PsubBlock">level</xsl:when>
			</xsl:choose>
		</xsl:with-param>
		<xsl:with-param name="hcontainer-name">
			<xsl:choose>
				<xsl:when test="self::Group">group</xsl:when>
				<xsl:when test="self::Pblock">cross-heading</xsl:when>
				<xsl:when test="self::PsubBlock">sub-heading</xsl:when>
...
<xsl:template match="Pblock/Title | PsubBlock/Title">
	<crossHeading>
		<xsl:apply-templates />
		<xsl:call-template name="reference" />
	</crossHeading>
